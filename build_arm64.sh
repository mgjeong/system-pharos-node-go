...
function build(){
    CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -o pharos-anchor -a -ldflags '-extldflags "-static"'  src/main/main.go
    if [ $? -ne 0 ]; then
...
echo -ne "\nMaking executable file of Pharos-Node service"
build
echo ": Done"
